...
import { X, CheckSquare, ImageIcon, Trash2 } from 'lucide-react';
import { Task, Key } from '../../../../../../types/Task'; // Ensure Key is imported
import { getTaskDetails, addTaskDescription, updateTaskDescription, getAllChecklists, createChecklist, assignLabelsToTask, removeLabelFromTask } from '../../../../../../services/ProjectTaskApi';
...
import { MemberSelectionModal } from '../TaskDetailsModal/memberSelectionModal';
...
const TaskModal: React.FC<TaskModalProps> = ({ task, onClose }) => {
  const [taskDetails, setTaskDetails] = useState<Task>({ ...task, status: task.status || 'pending' });
  ...
  const fetchTaskDetails = async (initialLoad: boolean = false) => {
    try {
      ...
      const [taskData, checklistsData] = await Promise.all([
  ...
  const toggleStatus = () => {
    const newStatus = taskDetails.status === 'pending' ? 'done' : 'pending';
  ...
  const handleAddAttachment = (attachment: any) => {
    setTaskDetails({
      ...taskDetails,
      attachments: [...(taskDetails.attachments || []), attachment],
    });
  };

  const handleDeleteAttachment = (attachmentId: string | number) => {
    setTaskDetails({
      ...taskDetails,
      attachments: (taskDetails.attachments || []).filter((attachment) => attachment.id !== attachmentId),
    });
  };

  const handleRemoveLabel = async (labelId: Key | null | undefined) => {
    if (labelId == null) return; // Guard against null/undefined

    try {
      // Convert labelId to string or number for the API
          ...
          <>
            ...
            {taskDetails.coverImage || taskDetails.coverColor ? (
              <div className="relative h-48 w-full overflow-hidden">
                ...
                <div className="absolute bottom-4 left-4 flex items-center gap-2 rounded-full bg-slate-900/80 px-3 py-1.5 shadow-md backdrop-blur-md transition-transform hover:scale-105">
                  <div className={`size-2.5 rounded-full ${getStatusColor(taskDetails.status)}`}></div>
                  ...
                  <Button
                    variant="ghost"
                    size="sm"
                    className="text-indigo-200 hover:bg-indigo-500/20"
                    onClick={toggleStatus}
                  >
                    {taskDetails.status === 'pending' ? 'Mark as Done' : 'Mark as Pending'}
                  </Button>
                </div>
              </div>
            ) : (
            ...
            <div className="sticky top-0 z-10 border-b border-indigo-500/20 bg-slate-900/95 backdrop-blur-md">
              <div className="px-6 py-4">
                ...
                <div className="ml-8 mt-2 flex items-center gap-2 text-sm text-slate-400">
